...
class Sampler(nn.Module):
    ...
    def __init__(self, vocab_size: int, org_vocab_size: Optional[int] = None) -> None:
        super().__init__()
...
def _apply_penalties(
    ...
    _, prompt_mask = _get_bin_counts_and_mask(
...
def _sample(
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            sample_results = _random_sample(seq_groups, is_prompts, multinomial_samples)
        elif sampling_type == SamplingType.BEAM:
            sample_results = _beam_search_sample(
                seq_groups, is_prompts, sampling_metadata.seq_data, beam_search_logprobs
            )
