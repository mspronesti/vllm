...
class RejectionSampler(nn.Module):
    ...
    def _batch_modified_rejection_sampling(
        ...
        recovered_token_ids = _multinomial(recovered_probs, num_samples=1).reshape(
            batch_size, k
        )
    ...
    def _raise_if_incorrect_dtype(
        ...
        assert all(
            probs.dtype == self.probs_dtype for probs in [target_probs, draft_probs]
        )
        assert all(
    ...
    def _raise_if_inconsistent_device(
        ...
        devices = [
            t.device
            for t in [target_probs, bonus_token_ids, draft_probs, draft_token_ids]
        ]
