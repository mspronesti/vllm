...
NUM_BLOCKS = (
    [1024, 36000] if not is_hip else [1024, 10000]
...
def test_swap_blocks(
    ...
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
    src_device = f"{direction[0]}:{device}" if direction[0] == "cuda" else direction[0]
    dst_device = f"{direction[1]}:{device}" if direction[1] == "cuda" else direction[1]
