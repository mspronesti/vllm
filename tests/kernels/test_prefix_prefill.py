...
def test_contexted_kv_attention(
    ...
    k_cache = torch.zeros(cache_size, block_size, num_heads, head_size, dtype=dtype)
    ...
    k_cache = (
        k_cache.view(-1, block_size, num_heads, head_size // 8, 8)
        .permute(0, 2, 3, 1, 4)
        .contiguous()
    )
    ...
    v_cache = (
        v_cache.view(-1, block_size, num_heads, head_size)
        .permute(0, 2, 3, 1)
        .contiguous()
    )
